...
def get_transcript(db, transcript_id):
    transcript = db.transcripts.find_one({'transcript_id': transcript_id}, {'_id': False})
    if not transcript:
        return None
...
def get_variants_from_dbsnp(db, rsid):
    ...
    if position:
        variants = list(db.variants.find({'xpos': {'$lte': position['xpos'], '$gte': position['xpos']}}, {'_id': False}))
        if variants:
            add_consequence_to_variants(variants)
            return variants
    return []
...
def get_metrics(db, variant):
    ...
    if metric is not None:
        metrics['Site Quality'] = db.metrics.find_one({'metric': 'binned_%s' % metric}, {'_id': False})
    return metrics
...
def get_variants_in_gene(db, gene_id):
    ...
    for variant in db.variants.find({'genes': gene_id}, {'_id': False}):
        ...
        pathogenicity = ''
        try:
            pathogenicity = [ann['pathogenicity'] for ann in variant['vep_annotations'] ]
            pathogenicity = pathogenicity[0]
        except:
            pathogenicity = ''
...
def get_exons_in_transcript(db, transcript_id):
    ...
    return sorted(list(db.exons.find({'transcript_id': transcript_id, 'feature_type': { "$in": ['CDS', 'UTR', 'exon'] }}, {'_id': False})), key=lambda k: k['start'])
