...
ADMINISTRATORS = (
    'honeyguide@gmail.com',
)

app = Flask(__name__)

app.config['SESSION_TYPE'] = 'memcached'
app.config['SECRET_KEY'] = '\xcf\x10\xb4Q#\xf3\xcc5D\xf6\xb63\xadFk\x02"\x8b|D\xf3\x08\xc3['
#sess = Session()
...
def homepageX():
    ...
    #if t is None:
    #    t = render_template('homepage.html',dbname=exac_var)
...
def homepage_artfl():
    tmpdbname = (request.path).replace('/','')
    session['db_selected'] = tmpdbname+'1'
    exac_var = tmpdbname.lower()
    cache_key = 't-homepage'
    #t = cache.get(cache_key)
    #if t is None:
    ...
    #    cache.set(cache_key, t)
    #return t
    summaryVepDict = getVCFSummary()
    print "------->>>",session['db_selected'], exac_var
    t = render_template('homepage.html',dbname=exac_var,summaryVepDict=json.dumps(summaryVepDict))
    return t
...
def getconfig():
    app.config.update(dict(
        ...
        CONSTRAINT_FILE=os.path.join(os.path.dirname(__file__), EXAC_FILES_DIRECTORY, 'fordist_cleaned_exac_r03_march16_z_pli_rec_null_data.txt.gz'),
...
def precalculate_metrics():
    ...
    for variant in db.variants.find(fields={'quality_metrics':True, 'site_quality':True, 'allele_num':True, 'allele_count':True}):#fields={"_id":False}
...
def get_gene_page_content(gene_id):
    ...
    try:
        ...
        if t is None:
            ...
            #print "variants_in_gene----->>>",variants_in_gene,"\nvariants_in_transcript----->>>",variants_in_transcript
            t = render_template(
                'gene.html',
                gene=gene,
