...
def main(args):
    ...
    for line in f:
        ...
        for metric in metrics:
            ...
            new_info += ';%s_MID=' % (metric) + midpoints
            new_info += ';%s_HIST=' % (metric) + hist
...
def get_histogram_for_variant(vcf_line, metric="DP", num_bins=10, midpoints=True, variants_only=False):
        ...
        for sample in fields[9:]:
            ...
            if sample_info[0] != './.':
                if idx < len(sample_info) and sample_info[idx] != '.':
                    distr_all_individuals.append(sample_info[idx])
            if sample_info[0] == '0/0':continue
            if idx < len(sample_info) and sample_info[idx] != '.':
                distr.append(sample_info[idx])
        #print metric,"---------",distr,"\nall_indiv",distr_all_individuals
        mids, hist = get_hist_from_distribution(distr, midpoints, num_bins)
        mids_all, hist_all = get_hist_from_distribution(distr_all_individuals, midpoints, num_bins)
        mids = map(str, mids)
        hist = map(str, hist)
        mids = '|'.join(map(str, mids))
        hist = '|'.join(map(str, hist))
        mids_all = map(str, mids_all)
        hist_all = map(str, hist_all)
        mids_all = '|'.join(map(str, mids_all))
        hist_all = '|'.join(map(str, hist_all))
        final_mids = mids+","+mids_all
        final_hist = hist+","+hist_all
        #return map(str, mids), map(str, hist)
        #print metric,"== ",final_mids,"------->>",final_hist
        return final_mids, final_hist
